...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in xrange(self.belief.getNumRows()):
            for col in xrange(self.belief.getNumCols()):
                dist = math.sqrt((util.rowToY(row) - agentY) ** 2 + (util.colToX(col) - agentX) ** 2)
    ...
    def elapseTime(self):
        ...
        for key, value in self.transProb.iteritems():
            newBelief.addProb(key[1][0], key[1][1], self.belief.getProb(key[0][0], key[0][1]) * value)
        newBelief.normalize()
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for tile, count in self.particles.iteritems():
            dist = math.sqrt((util.rowToY(tile[0]) - agentY) ** 2 + (util.colToX(tile[1]) - agentX) ** 2)
            print agentX, agentY
            print tile, util.rowToY(tile[0]), util.colToX(tile[1]), dist 
            likelihood = util.pdf(dist, Const.SONAR_STD, observedDist)
            weights[tile] = count * likelihood
    ...
    def elapseTime(self):
        ...
        for tile, count in self.particles.iteritems():
            for _ in xrange(count):
                resample[util.weightedRandomChoice(self.transProbDict[tile])] += 1
