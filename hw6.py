...
def create_chain_csp(n):
    ...
    for k in xrange(n):
        csp.add_variable(variables[k], domain)
    for k in xrange(n - 1):
        csp.add_binary_factor(variables[k], variables[k+1], lambda x, y: x != y)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    domain = range(1, n + 1)
    for p in xrange(n):
        csp.add_variable(variables[p], domain)
    for k in xrange(n):
        for j in xrange(k + 1, n):
            csp.add_binary_factor(variables[k], variables[j], lambda x, y: x!= y)
            csp.add_binary_factor(variables[k], variables[j], lambda x, y: abs(x - y) != abs(k - j))
    # END_YOUR_CODE
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
            feasible = {var : 0 for var in self.csp.variables if var not in assignment}
            for var in sorted(feasible.keys()):
                for val in self.domains[var]:
                    if self.get_delta_weight(assignment, var, val) != 0:
                        feasible[var] += 1
            selected = sorted(feasible.items(), key = lambda (k, v): (v, k))
            return selected[0][0]
            # END_YOUR_CODE
    ...
    def arc_consistency_check(self, var):
        ...
        while checking:
            ...
            for var2 in self.csp.get_neighbor_vars(var1):
                ...
                newdomain2 = set(domain2)
                for val2 in domain2:
                    if self.csp.unaryFactors[var2] and self.csp.unaryFactors[var2][val2] == 0:
                        newdomain2.remove(val2)
                        break
                    b = 0
                    for val1 in domain1:
                        if self.csp.binaryFactors[var1][var2]:
                            b += self.csp.binaryFactors[var1][var2][val1][val2]
                    if not b: newdomain2.remove(val2)
                if newdomain2 != set(domain2):
                    checking.append(var2)
                    self.domains[var2] = list(newdomain2)
        # END_YOUR_CODE
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    for i in xrange(maxSum + 1):
        for j in xrange(i, maxSum + 1):
            Bdomain.append((i, j))
    Bvariables = [('sum', name, var) for var in variables]
    for Bvar in Bvariables:
        csp.add_variable(Bvar, Bdomain)
    for var in variables:
        csp.add_binary_factor(var, ('sum', name, var), lambda x, y: y[0] + x == y[1])
    csp.add_unary_factor(('sum', name, variables[0]), lambda x: x[0] == 0)
    for k in xrange(len(variables) - 1):
        csp.add_binary_factor(('sum', name, variables[k]), ('sum', name, variables[k + 1]), lambda x, y: x[1] == y[0])
    csp.add_binary_factor(result, ('sum', name, variables[-1]), lambda x, y: x == y[1])
    return result
    # END_YOUR_CODE
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for req in self.profile.requests:
            for quarter in self.profile.quarters:
                csp.add_unary_factor((req, quarter), lambda cid: cid is None or len(req.quarters) == 0 or quarter in req.quarters)
        # END_YOUR_CODE
    ...
    def add_unit_constraints(self, csp):
        ...
        for cid in cids:
            for quarter in self.profile.quarters:
                ...
                csp.add_variable((cid, quarter), range(course.minUnits, course.maxUnits + 1) + [0])
        def matchUnits(cid1, units, cid2):
            return (cid1 is None and units == 0) or (cid1 == cid2 and units != 0) or (cid1 != cid2 and units == 0)
        for quarter in self.profile.quarters:
            for req in self.profile.requests:
                for cid in req.cids:
                    csp.add_binary_factor((req, quarter), (cid, quarter), lambda x, y: matchUnits(x, y, cid))
        for quarter in self.profile.quarters:
            currentVars = [(cid, quarter) for cid in cids]
            currentsum = get_sum_variable(csp, quarter, currentVars, self.profile.maxUnits)
            csp.add_unary_factor(currentsum, lambda x: x >= self.profile.minUnits)
        # END_YOUR_CODE
