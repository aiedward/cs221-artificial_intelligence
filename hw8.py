...
def formula2d():
    ...
    And(Exists('$z', And(Parent('$x', '$z'), Parent('$z', '$y'))), Female('$y')))))
    # END_YOUR_CODE
...
def ints():
    ...
    formulas.append(Forall('$x', Exists('$y', And(Not(Equals('$y', '$x')), And(Successor('$x', '$y'), Forall('$z', Implies(Not(Equals('$y', '$z')), Not(Successor('$x', '$z')))))))))
...
def createRule3():
    ...
    return GrammarRule('$Clause', ['if', 'a', '$Noun', '$Verb', 'a', '$Noun', 'then', 'the', 'former', '$Verb', 'the', 'latter'],
        ...
        (AndList([Atom(args[0].title(), '$x'), Atom(args[2].title(), '$y'), Atom(args[1].title(), '$x', '$y')]), \
